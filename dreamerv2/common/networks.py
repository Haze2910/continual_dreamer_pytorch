...
class EnsembleRSSM(nn.Module):
    def __init__(
        ...
        super().__init__()
        self._device = device
        self._ensemble = ensemble # number of TODO
        ...
        self._discrete = discrete # set to False to use Gaussian latent variables, to a Int to use categorical latent variables
    ...
    def get_feat(self, state):
        """
        Get stochastic and deterministic tensors
        """
        stoch = state["stoch"]
        if self._discrete:
...
class Decoder(nn.Module):
    """
    Decoder for the encoded observation image
    """
    def __init__(
